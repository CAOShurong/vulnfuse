...
It provides defensive parsers, a canonical evidence model, deterministic explainable matching, clustering, scanner coverage/overlap analytics, scan-set-aware cross-run baseline comparison, and JSON/SARIF/CSV/Markdown/self-contained HTML exporters. The public package is not yet published to npm; use the repository workspace or the paired GitHub release packages.

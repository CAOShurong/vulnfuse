...
It provides defensive parsers, including bounded CycloneDX JSON/XML VDR/VEX ingestion, a canonical evidence model, deterministic explainable matching, clustering, portable SARIF URI-base prefixes, SARIF result-kind and suppression evidence with active/suppressed/non-finding disposition, scanner coverage/overlap analytics, scan-set-aware cross-run baseline comparison, and JSON/SARIF/CSV/Markdown/self-contained HTML exporters. CycloneDX XML rejects DTDs and maps supported evidence rather than performing full XSD validation. The public package is not yet published to npm; use the repository workspace or the paired GitHub release packages.

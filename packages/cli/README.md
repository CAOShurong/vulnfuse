...
Baseline diffs record and warn about added/removed scanner tools, per-tool report-count changes, and changed or missing embedded producer-version evidence. Add `--fail-on-scan-set-change` to make that warning fail CI after the complete comparison has been written. Versions are compared as literal evidence, not ordered as upgrades. Equal names, counts, and versions do not prove that scan targets, settings, rules, vulnerability databases, or execution were identical.

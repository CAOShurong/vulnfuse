...
1. active status before effectively suppressed status before non-finding status;
2. highest severity;
3. most identifiers and populated descriptive/component fields;
4. lexicographically smallest stable finding ID.

SARIF result kind and suppression do not change whether records correlate. They change the cluster disposition after correlation. A cluster is non-finding only when every member is non-finding; otherwise it is effectively suppressed only when every actual-finding member is suppressed. This prevents pass evidence or a suppressed duplicate from hiding active evidence reported by another scanner.
...
- `updated`: a baseline cluster matched, but severity, disposition, title, kind, identifiers, components, assets, locations, remediation, source tools, or source-record count changed;
...
Every comparison also records a `scanSetChange` object. It lists scanner tools present only in one run, report-count changes for tools present in both, changed embedded producer-version evidence, and changed SARIF automation-category evidence. Version evidence contains a sorted literal version set plus the number of unversioned reports for that tool. SARIF category evidence contains the sorted category prefixes extracted from `run.automationDetails.id` plus the number of runs without a category, so known-to-unknown and unknown-to-known changes remain explicit even for zero-result runs. GitHub interprets everything before the final `/` as the category; an id without `/` is only a run id and remains uncategorized here. CLI, Action, browser, Markdown, SARIF, row-bearing CSV, and HTML surfaces warn when that structure reports drift. An empty CSV has no data row on which to repeat report-level metadata, so use JSON, Markdown, SARIF, or HTML when that distinction must survive an empty comparison. The opt-in CLI/Action scan-set gate fails only after writing the comparison. VulnFuse does not order versions, infer an upgrade, interpret category hierarchy, or claim actual scope. Equal tool names, counts, embedded versions, and categories do not prove equal targets, settings, rule sets, databases, or successful scan completion.
...
`newBySeverity` counts every new cluster for audit reporting. `newActiveBySeverity` excludes fully suppressed and fully non-finding new clusters and is the input to `--fail-on-new` and the corresponding Action gate.

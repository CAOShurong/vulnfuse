...
Every comparison also records a `scanSetChange` object. It lists scanner tools present only in one run, report-count changes for tools present in both, and changed embedded producer-version evidence. Version evidence contains a sorted literal version set plus the number of unversioned reports for that tool, so known-to-unknown and unknown-to-known changes remain explicit. CLI, Action, browser, Markdown, SARIF, row-bearing CSV, and HTML surfaces warn when that structure reports drift. An empty CSV has no data row on which to repeat report-level metadata, so use JSON, Markdown, SARIF, or HTML when that distinction must survive an empty comparison. The opt-in CLI/Action scan-set gate fails only after writing the comparison. VulnFuse does not order versions or infer an upgrade. Equal tool names, counts, and embedded versions do not prove equal targets, settings, rule sets, databases, or successful scan completion.

...
12. **A stable scan set is not proven comparable.** VulnFuse warns when tool names, per-tool report counts, or embedded producer-version evidence changes. Equal names, counts, and versions still cannot establish identical assets, scanner configuration, rules, advisory databases, or successful scan completion. Versions are unauthenticated producer strings; retain scanner logs, database/rule provenance, and target identity for consequential baseline decisions.

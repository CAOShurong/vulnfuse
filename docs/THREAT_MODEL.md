...
24. **Trivy SARIF image identity is producer-controlled.** VulnFuse accepts one unambiguous SHA-256 container reference from a Trivy-named run and can use it with an exact vulnerability identifier to group product-scoped OpenVEX evidence. A malicious report can lie about both fields and influence clustering. Ambiguous or malformed references are not converted to PURLs, OpenVEX status remains active evidence, and no grouping proves that the image was scanned, the VEX author was trusted, or the assertion was correct.
